...
from app.scheduler.file_processor import process_pending_files

def main():
    config = get_config()
    init_db(config.database_url)
    
    print("Обработка файлов с исправленным парсером...")
    with get_session() as db:
        count = process_pending_files(db)
